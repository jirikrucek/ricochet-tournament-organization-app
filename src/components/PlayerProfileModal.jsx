import React, { useMemo } from 'react';
import { useTranslation } from 'react-i18next';
import { X, User, Trophy, Medal, Eye } from 'lucide-react';
import { getCountryCode } from '../constants/countries';
import { usePlayerFocus } from '../contexts/PlayerFocusContext';
import { useNavigate } from 'react-router-dom';

const PlayerProfileModal = ({ player, matches, allPlayers, onClose }) => {
    const { t } = useTranslation();
    const { enableFocus } = usePlayerFocus();
    const navigate = useNavigate();

    const handleFocusMode = () => {
        enableFocus(player.id);
        onClose();
        navigate('/matches');
    };

    const playerMatches = useMemo(() => {
        if (!matches || !player) return [];
        /* ... existing logic ... */
        const filtered = matches.filter(m =>
            m.player1Id === player.id || m.player2Id === player.id
        );

        return filtered.map(m => {
            const isP1 = m.player1Id === player.id;
            const opponentId = isP1 ? m.player2Id : m.player1Id;
            const opponent = allPlayers.find(p => p.id === opponentId) || { full_name: 'TBD' };

            const myScore = isP1 ? m.score1 : m.score2;
            const opScore = isP1 ? m.score2 : m.score1;

            let result = 'pending';
            if (m.winnerId) {
                result = m.winnerId === player.id ? 'win' : 'loss';
            }

            return {
                ...m,
                opponent,
                myScore,
                opScore,
                result,
                isP1
            };
        }).sort((a, b) => {
            // Sort by status: live > pending > finished
            const scoreStatus = (m) => m.status === 'live' ? 0 : (m.status === 'pending' ? 1 : 2);
            return scoreStatus(a) - scoreStatus(b);
        });
    }, [matches, player, allPlayers]);

    // Stats ...
    const stats = useMemo(() => {
        const played = playerMatches.filter(m => m.result !== 'pending'); // actually "finished" check better but result works
        const wins = played.filter(m => m.result === 'win').length;
        const losses = played.filter(m => m.result === 'loss').length;
        return { wins, losses, played: played.length };
    }, [playerMatches]);

    if (!player) return null;

    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content profile-modal" onClick={e => e.stopPropagation()}>
                <div className="profile-header-bg">
                    <div style={{ position: 'absolute', top: '1rem', right: '1rem', display: 'flex', gap: '0.5rem' }}>
                        <button onClick={handleFocusMode} className="close-button light" title={t('profile.focusMode') || "Focus Mode"}>
                            <Eye size={24} />
                        </button>
                        <button onClick={onClose} className="close-button light">
                            <X size={24} />
                        </button>
                    </div>

                    <div className="profile-hero">
                        <div className="profile-avatar-large">
                            {player.photo ? (
                                <img src={player.photo} alt={player.full_name} />
                            ) : (
                                <User size={48} />
                            )}
                        </div>
                        <div className="profile-info">
                            <h2 className="profile-name">{player.full_name}</h2>
                            <div className="profile-meta">
                                {player.country && (
                                    <div className="meta-item">
                                        {/* Flag removed or kept based on design preferences, keeping simple text/icon here */}
                                        <span>{player.country}</span>
                                    </div>
                                )}
                                {player.elo && (
                                    <div className="meta-item badge-elo">
                                        <Trophy size={14} />
                                        <span>POINTS {player.elo}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    </div>
                </div>

                <div className="profile-body">
                    <div className="stats-row">
                        <div className="stat-card">
                            <span className="stat-value">{stats.played}</span>
                            <span className="stat-label">{t('profile.matchesPlayed')}</span>
                        </div>
                        <div className="stat-card mobile-green">
                            <span className="stat-value">{stats.wins}</span>
                            <span className="stat-label">{t('profile.wins')}</span>
                        </div>
                        <div className="stat-card mobile-red">
                            <span className="stat-value">{stats.losses}</span>
                            <span className="stat-label">{t('profile.losses')}</span>
                        </div>
                    </div>

                    <h3 className="section-title-small">{t('profile.matchHistory')}</h3>

                    <div className="history-list">
                        {playerMatches.length > 0 ? (
                            playerMatches.map(match => (
                                <div key={match.id} className={`history-item ${match.status === 'live' ? 'live-item' : match.result}`}>
                                    <div className="history-info">
                                        <div className="history-bracket">
                                            {(match.bracket || '').toUpperCase()} {match.round ? `R${match.round}` : ''}
                                        </div>
                                        <div className="history-opponent">
                                            <span style={{ color: 'var(--text-secondary)', marginRight: '4px' }}>{t('common.vs')}</span>
                                            {formatName(match.opponent)}
                                        </div>
                                    </div>
                                    <div className="history-score-box">
                                        {match.status === 'live' && <span className="live-tag">LIVE</span>}
                                        <div className="main-score">
                                            <span className={match.result === 'win' ? 'score-win' : ''}>{match.myScore ?? 0}</span>
                                            <span>:</span>
                                            <span className={match.result === 'loss' ? 'score-loss' : ''}>{match.opScore ?? 0}</span>
                                        </div>
                                    </div>
                                    <div className="result-badge">
                                        {match.status === 'live' ? '•' : (match.result === 'win' ? 'W' : (match.result === 'loss' ? 'L' : '-'))}
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="empty-history">{t('profile.noMatches')}</div>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
};

// Start Helper
const formatName = (p) => {
    if (!p) return 'TBD';
    if (p.full_name) return p.full_name;
    return 'Unknown';
}

export default PlayerProfileModal;
